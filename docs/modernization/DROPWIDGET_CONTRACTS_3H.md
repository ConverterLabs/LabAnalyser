...
| `QSliderD`: ctor, drag enter/move/drop, five virtual methods, `contextMenu`, `RemoveConnection`, `RequestUpdate` | ctor/numeric adapter path direct DW_001, DW_002 | Actual dropped min/max, slider user value signal, context/removal and errors not run; 3I. | High: rounding/range + user signal contract. |
...
| DW_016 | `QPushButtonD::GetVariantData`; slider/spin clamping, repeat-value signal suppression and scaled slider roundtrip; button, check, combo and line-edit signal count/order/payload. | `QPushButtonD::StartTimeOut` requires a manager mapping when its delayed path queries a container; this is a 3I MainWindow boundary, not run standalone. |
