...
| TCP remote control | `RemoteControl/RemoteControlServer.*`, private `RemoteControlFrameSplitter.*` and `RemoteControlProtocol.*` | Loopback port fallback; native binary framing; `set`/`get` bytes, signals, fragmentation, disconnect, current-socket behavior and safe decode/response byte vectors. With two live clients, only the last accepted socket is processed; accepting B clears A's fragment state and later A input is not processed. The helpers are QObject-free and do not own sockets, mappers or Messenger state. | `contract/RemoteControlContractTests::{TCP_001..TCP_012}` | connection-state characterization complete; accepted server-socket lifetime, unsafe frames and platform faults unverified |
