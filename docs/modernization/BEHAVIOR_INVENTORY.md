...
| TCP remote control | `RemoteControl/RemoteControlServer.*`, private `RemoteControlConnectionState.*`, `RemoteControlFrameSplitter.*` and `RemoteControlProtocol.*` | Loopback port fallback; native binary framing; `set`/`get` bytes, signals, fragmentation, disconnect, current-socket behavior and safe decode/response byte vectors. `TCP_001..TCP_008` cover lifecycle, command/response bytes, safe fragmentation and reconnect; `TCP_009..TCP_012` cover last-accepted-client transition, discarded old remainder, reply routing and safe client/server teardown. `TCP_013` proves the Qt-6 `errorOccurred` meta-signal, absent legacy `error` signal and accept-path warning. `TCP_014..TCP_015` prove accepted sockets are `QTcpServer` children that remain after disconnect until server destruction. With two live clients, only the last accepted socket is processed; accepting B clears A's fragment state and later A input is not processed. ConnectionState observes but never owns the current socket; protocol/splitter remain QObject-free. | `contract/RemoteControlContractTests::{TCP_001..TCP_015}` | 5A.1 characterization complete. `TCP_016` socket-error delivery is not deterministically verified. Qt-6 signal repair, accepted-socket cleanup, unsafe frames and platform faults require approved hardening. |
