...
| Experiment XML | `LoadSave/xmlexperimentreader.*`, `xmlexperimentwriter.*`; `UIDataManagementSetClass::{LoadExperiment,SaveExperiment}` | Read/write `Experiment` elements (Tabs, Devices, Widgets, State, FigureWindows, Connections), relative paths and Qt dock state. Reader/writer use `false` for success; unknown XML is skipped. `errorString()` is empty after parse errors. Excess `PlotWidgetName` elements now deterministically raise a parser error before list indexing; fewer names retain generated names and partial state is intentionally not rolled back. Figure rows/cols are validated before GUI creation: each is `0..32`, product is at most 256; zero grids remain valid and invalid input returns `true` with deterministic parser text. | `contract/XmlExperimentContractTests::{XML_001..XML_008, XML_FIG_001..XML_FIG_008}` | legacy fixture corpus covered; accepted geometry/allocation and rollback after later malformed figure children remain exclusions |
...
`XML_FIG_001..XML_FIG_007` map the real MainWindow-backed figure reader. They
cover exact, fewer and absent names; unknown child skipping; safe empty grids;
and the approved excess-name parser failure with retained partial state. The
exact parser text is not observable through public `errorString()`, which
remains empty. Oversized dimensions, allocation failure and rollback after a
partial figure remain documented dangerous exclusions.
...
## MATLAB remote connector

| Production boundary | Observable contract | Tests | Status / risk |
| --- | --- | --- | --- |
| `MatlabRemoteConnector` exported C API and TCP codec | Eight unmangled DLL functions, including additive `IsConnected`; loopback-only `127.0.0.1`; port supplied as text; native `uint32_t`/`double` framing; scalar-double and NUL-terminated text sets; synchronous numeric/padded-text gets; absolute two-second I/O deadline; failed/incomplete connections are discarded; per-port state; legacy send return value zero. MATLAB `Get` preserves exact-ID results and returns a `containers.Map` of `x`/`y` structs for IDs containing `*`. | `MatlabRemoteConnectorContractTests`; `MatlabPackageSourceContract`; `RemoteControlContractTests::TCP_032_wildcardIdEnumeration` | Characterized before the RAII/Winsock refactor and timeout hardening, then passed afterward. `TCP_032` recorded literal-star empty responses before the approved wildcard behavior and now covers all, contains, prefix, suffix, no-match and repeated-star patterns. Silent peer, partial reply, immediate close, null inputs, repeated disconnect and oversized replies are safe paths. Real MATLAB loading remains unverified. |
