...
| TCP remote control | `RemoteControl/RemoteControlServer.*`, private `RemoteControlFrameSplitter.*` and `RemoteControlProtocol.*` | Loopback port fallback; native binary framing; `set`/`get` bytes, signals, fragmentation, disconnect, current-socket behavior and safe decode/response byte vectors. The helpers are QObject-free and do not own sockets, mappers or Messenger state. | `contract/RemoteControlContractTests::{TCP_001..TCP_008}` | baseline characterized; unsafe short frames and platform faults unverified |
