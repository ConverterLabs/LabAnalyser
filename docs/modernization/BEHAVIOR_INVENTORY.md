...
| TCP remote control | `RemoteControl/RemoteControlServer.*` | Loopback port fallback; native binary framing; `set`/`get` bytes, signals, fragmentation, disconnect, current-socket behavior and safe decode/response byte vectors. | `contract/RemoteControlContractTests::{TCP_001..TCP_008}` | baseline characterized; unsafe short frames and platform faults unverified |
