...
| TCP remote control | `RemoteControl/RemoteControlServer.*`, private `RemoteControlConnectionState.*`, `RemoteControlFrameSplitter.*` and `RemoteControlProtocol.*` | Loopback port fallback; native binary framing; `set`/`get` bytes, signals, fragmentation, disconnect, current-socket behavior and safe decode/response byte vectors. `TCP_001..TCP_008` cover lifecycle, command/response bytes, safe fragmentation and reconnect; `TCP_009..TCP_012` cover last-accepted-client transition, discarded old remainder, reply routing and safe client/server teardown. `TCP_013` proves the Qt-6 `errorOccurred` meta-signal, absent legacy `error` signal and absence of the prior accept-path warning after the approved typed connection fix. `TCP_014..TCP_015` prove accepted sockets are `QTcpServer` children that remain after disconnect until server destruction. With two live clients, only the last accepted socket is processed; accepting B clears A's fragment state and later A input is not processed. ConnectionState observes but never owns the current socket; protocol/splitter remain QObject-free. | `contract/RemoteControlContractTests::{TCP_001..TCP_015}` | 5A.2a Qt-6 connection correction complete. `displayError` remains a no-op; `TCP_016` socket-error delivery and accepted-socket cleanup remain unverified/approval-required. Unsafe frames and platform faults remain excluded. |
