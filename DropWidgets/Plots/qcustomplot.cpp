...
void QCPGraph::getPlotData(QVector<QPointF> *lineData, QVector<QCPData> *scatterData) const
{
  switch(mLineStyle)
  {
    ...
    default: getLinePlotData(lineData, scatterData); break;
  }
}
...
void QCPGraph::getLinePlotData(QVector<QPointF> *lineData, QVector<QCPData> *pointData) const
{
     ...
     if(this->DataPointerY == NULL)
         return;
     int first = 1;
     int last = this->DataPointerX->size();
     if(!XYPlot())
     {
           first =std::lower_bound(this->DataPointerX->begin(),this->DataPointerX->end(),mKeyAxis.data()->range().lower+XOffset)-this->DataPointerX->begin();
           last = std::upper_bound(this->DataPointerX->begin(),this->DataPointerX->end(),mKeyAxis.data()->range().upper+XOffset)-this->DataPointerX->begin();
     }
     if(this->DataPointerX->size())
     {
     ...
     if (dataCount > 0)
     {
       ...
       if (keyAxis->orientation() == Qt::Vertical)
       {
           if(this->DataPointerX->size())
           {
               ...
               {
                 if (pointData)
                (*pointData)[i] = QCPData((this->DataPointerX->data())[first+i*modFactor] - this->XOffset,(this->DataPointerY->data())[first+i*modFactor]);
                 if (i!=dataCount) {

                     (*lineData)[i].setX(keyAxis->coordToPixel((this->DataPointerY->data())[first+i*modFactor]));
                     (*lineData)[i].setY(valueAxis->coordToPixel((this->DataPointerX->data())[first+i*modFactor]));
                 }
               }
           }
           else
       ...
       } else // key axis is horizontal
       {
           if(this->DataPointerX->size())
           {
               ...
               for(int i = 0; i < dataCount; i++)
               {
                   double y_s = (this->DataPointerY->data())[first+i*modFactor];
                   double x_s = (this->DataPointerX->data())[first+i*modFactor] - XOffset;
                   ...
                   if(gt==true)
                   {
                       ...
                       for(int k = 0; k < modFactor; k += std::ceil((double)modFactor/((double)100)))
                           {
                                if(y_s > (this->DataPointerY->data())[first+i*modFactor+k])
                                {
                                    y_s = (this->DataPointerY->data())[first+i*modFactor+k];
                                    x_s = (this->DataPointerX->data())[first+i*modFactor+k] - XOffset;
                                }
                           }
                   }
                   else
                   ...
                   if (pointData)
                   {
                        (*pointData)[i] = QCPData((this->DataPointerX->data())[first+i*modFactor] - XOffset,(this->DataPointerY->data())[first+i*modFactor]);
                   }
                   if (i!=dataCount) {
