...
void QCPAbstractPlottable::setID(const QString &name)
{
  mID = name;
}
...
{
  return mSelected ? mBrush : mBrush;
}
  ...
  QCPAbstractPlottable(keyAxis, valueAxis)
{
  ...
  setBrush(Qt::NoBrush);
  setSelectedPen(QPen(QColor(80, 80, 255), 1));
...
void QCPGraph::setData(QCPDataMap *data,std::set<double,QMap::iterator> WrapperSetI, bool copy)
{
  if (mData == data)
  {
...
/*! \overload
  ...
  smallest vector.
*/
void QCPGraph::setData( boost::shared_ptr<std::vector<double>> key,   boost::shared_ptr<std::vector<double>> value, double Xoffset_)
{
        this->DataPointerX  = key;
        this->DataPointerY  = value;
        this->XOffset  = Xoffset_;
}

void QCPGraph::setFFTData( boost::shared_ptr<std::vector<double>> key,   boost::shared_ptr<std::vector<double>> value)
{
        this->DataFFTX  = key;
        this->DataFFTY  = value;
}
...
{
  Q_UNUSED(details)
  if ((onlySelectable && !mSelectable) /*|| mData->isEmpty()*/) //todo
    return -1;
...
void QCPGraph::draw(QCPPainter *painter)
{
  ...
  if (mKeyAxis.data()->range().size() <= 0/* || mData->isEmpty()*/) return; //todo
...
void QCPGraph::getPlotData(QVector<QPointF> *lineData, QVector<QCPData> *scatterData) const
{
  switch(mLineStyle)
  {
    ...
    default: getFFTPlotData(lineData, scatterData); break;
  }
}
...
void QCPGraph::getFFTPlotData(QVector<QPointF> *lineData, QVector<QCPData> *pointData) const
{
    if(DataFFTX == nullptr)
        return;
    int dataCount =  DataFFTX->size();

    QCPAxis *keyAxis = mKeyAxis.data();
    QCPAxis *valueAxis = mValueAxis.data();

    lineData->reserve(dataCount+2); // added 2 to reserve memory for lower/upper fill base points that might be needed for fill
    lineData->resize(dataCount);    // Micha: restored to normal
    ...
    if(DataFFTX->size())
    {
      ...
      {
        if (pointData)
           (*pointData)[i] = QCPData(DataFFTX->at(i), DataFFTY->at(i));
        if (i != dataCount)
        {
           (*lineData)[i].setX(keyAxis->coordToPixel(DataFFTX->at(i)));
           (*lineData)[i].setY(valueAxis->coordToPixel(DataFFTY->at(i)));
        }
      }
    }
}
...
void QCPGraph::getLinePlotData(QVector<QPointF> *lineData, QVector<QCPData> *pointData) const
{
     ...
     int dataPointThreshold = 5760; // Micha: Initial threshold
     int dataCount = 0;

     if(this->DataPointerX == NULL)
         return;
     if(this->DataPointerY == NULL)
         return;

     auto LocalDataPointerX = DataPointerX;
     auto LocalDataPointerY = DataPointerY;
     auto LocalOffset = this->XOffset;

     int first = 1;
     int last = LocalDataPointerX->size();
     if(!XYPlot())
     {
           first =std::lower_bound(LocalDataPointerX->begin(),LocalDataPointerX->end(),mKeyAxis.data()->range().lower+LocalOffset)-LocalDataPointerX->begin();
           last = std::upper_bound(LocalDataPointerX->begin(),LocalDataPointerX->end(),mKeyAxis.data()->range().upper+LocalOffset)-LocalDataPointerX->begin();
     }
     if(LocalDataPointerX->size())
     {
        dataCount = last-first;
     }
     ...
     // Micha: dynamically adapt threshold
       if (dataCount > 500000)
           modFactor = 500; //dataPointThreshold = 1000;
       else if (dataCount > 400000)
           modFactor = 400; //dataPointThreshold = 1000;
     else if (dataCount > 300000)
         modFactor = 300; //dataPointThreshold = 1000;
     else if (dataCount > 200000)
       modFactor = 200; //dataPointThreshold = 1000;
     else if (dataCount > 100000)
         modFactor = 100; //dataPointThreshold = 1000;
     else if (dataCount > 50000)
         modFactor = 50; //dataPointThreshold = 1000;
     else if (dataCount > 10000)
         modFactor = 10; //dataPointThreshold =1000;
     else if (dataCount > 5000)
         modFactor = 1; //dataPointThreshold = 5760;

     //if(dataCount > dataPointThreshold)    //BG
     //  modFactor = (dataCount / dataPointThreshold);    //BG


    if(XYPlot())
        modFactor = 1;

     dataCount/=modFactor;    //BG
     ...
     if (dataCount > 0)
     {
       ...
       if (keyAxis->orientation() == Qt::Vertical)
       {
           if(LocalDataPointerX->size())
           {
               ...
               {
                 if (pointData)
                    (*pointData)[i] = QCPData((LocalDataPointerX->data())[first+i*modFactor] - LocalOffset,(LocalDataPointerY->data())[first+i*modFactor]);
                 if (i!=dataCount) {

                     (*lineData)[i].setX(keyAxis->coordToPixel((LocalDataPointerY->data())[first+i*modFactor]));
                     (*lineData)[i].setY(valueAxis->coordToPixel((LocalDataPointerX->data())[first+i*modFactor]));
                 }
               }
           }
           else
       ...
       } else // key axis is horizontal
       {
           if(LocalDataPointerX->size())
           {
               ...
               for(int i = 0; i < dataCount; i++)
               {
                   double y_s = (LocalDataPointerY->data())[first+i*modFactor];
                   double x_s = (LocalDataPointerX->data())[first+i*modFactor] - XOffset;

                   if(gt==true)
                   {
                       gt = false;
                       for(int k = 0; k < modFactor; k += std::ceil((double)modFactor/((double)100)))
                           {
                                if(y_s > (LocalDataPointerY->data())[first+i*modFactor+k])
                                {
                                    y_s = (LocalDataPointerY->data())[first+i*modFactor+k];
                                    x_s = (LocalDataPointerX->data())[first+i*modFactor+k] - XOffset;
                                }
                           }
                   }
                   else
                   ...
                   if (pointData)
                   {
                        (*pointData)[i] = QCPData((LocalDataPointerX->data())[first+i*modFactor] - XOffset,(LocalDataPointerY->data())[first+i*modFactor]);
                   }
                   if (i!=dataCount) {
                       (*lineData)[i].setY(valueAxis->coordToPixel(y_s));
                       (*lineData)[i].setX(keyAxis->coordToPixel(x_s));
                   }
               }
           }
           else
